...
# Descritor de forma
# A forma dos objetos se refere às características dos contornos dos objetos de interesse em uma imagem    
def extractHuMomentsFeatures(images):
    ...
    for image in images:
        if(len(image.shape)>2): #imagem colorida
            ...
            features = cv2.HuMoments(cv2.moments(blur_image)).flatten()
            featuresList.append(features)
            bar.next()
    bar.finish()
    return np.array(featuresList,dtype=object) # lista com vetores de características de todas as imagens
