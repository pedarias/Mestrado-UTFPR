...
def main():
    ...
    trainFeaturePath = './features_labels/haralick/train/'
    testFeaturePath = './features_labels/haralick/test/'
...
# As texturas se referem às diferenças nos níveis de intensidade, levando-se em
#consideração as regiões onde essas diferenças ocorrem e a direção dessas diferenças
def extractHaralickFeatures(images):
    bar = Bar('[INFO] Extrating Haralick features...',max=len(images),suffix='%(index)d/%(max)d  Duration:%(elapsed)ds')
    featuresList = []
    for image in images:
        if(len(image.shape)>2): #imagem colorida
            image = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
            blur_image = cv2.medianBlur(image,3)
            features = mahotas.features.haralick(blur_image).mean(axis=0)
